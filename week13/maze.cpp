...
void solveMaze()
{
   ...
   auto mazeboard = readMaze(fileName.c_str());
   auto path = custom::vector<Vertex>{};
   drawMaze(mazeboard, path);
   std::cout << "Press any key to solve the maze.\n";
   std::cin.ignore();
   std::cin.get();
   auto start = CVertex{0, 0};
   auto end = CVertex{start.getMaxCol() - 1, start.getMaxRow() - 1};
   path = mazeboard.findPath(start, end);
   drawMaze(mazeboard, path);
}
