 ...
 * Author:
 *    Sean Hawkes and Guillermo Oliva
 * Summary:
...
void BST<T>::erase(iterator pos)
{
   ...
   if (node->pLeft == nullptr && node->pRight == nullptr)
   {
      if (node != root_)
      {
         // invalidate pointer from parent
         if (par->pLeft == node)
            par->pLeft = nullptr;
         else
            par->pRight = nullptr;
      ...
      node = nullptr; // invalidate node pointer
   }
   // if both children exist
   else if (node->pLeft != nullptr && node->pRight != nullptr)
   {
      // find the smallest node on this branch
      auto min = findBranchMin(node->pRight);
      auto value = min->data; // store minimum node value
      erase(BST<T>::iterator{min}); // recursively call delete on minimum node
      node->data = value; // replace current value of node with min value
   }
   else
      // one of the children exists
   {
      // find which child exists and store it
      auto child = (node->pLeft != nullptr) ? node->pLeft : node->pRight;
      if (node != root_) // if not root
      {
         if (node == par->pLeft) // if node to be deleted is the left of parent
            par->pLeft = child; // replace left pointer with child
         else
            par->pRight = child;
         child->pParent = par; // else replace the right pointer
      }
      else
         root_ = child; // otherwise we are at root
      delete node; // delete node
      node = nullptr; // invalidate pointer
   }
   --numElements_; // change size
}
